...
class HL7ADTMessageTest {
    ...
    void testAdtA01FullWithObxTypeTXAndNoGroups(String message) throws IOException {
        ...
        assertThat(allergyIntoleranceResource).hasSize(0); // AL1 Segment unsupported
    ...
    void testAdtA01FullPlusMultipleProcedureGroupAndSingleInsurance(String message) throws IOException {
        ...
        // Don't support OBX segment in CentralRegion ADT messages
        List<Resource> observationResource = ResourceUtils.getResourceList(e, ResourceType.Observation);
        assertThat(observationResource).hasSize(0); 

        List<Resource> allergyIntoleranceResource = ResourceUtils.getResourceList(e, ResourceType.AllergyIntolerance);
        assertThat(allergyIntoleranceResource).hasSize(0); // We don't support AL1 segment in CentralRegion
        ...
        assertThat(coverages).hasSize(0); // We don't support INSURANCE.IN1 in CentralRegion

        List<Resource> organizationResource = ResourceUtils.getResourceList(e, ResourceType.Organization);
        assertThat(organizationResource).hasSize(0); // No INSURANCE.IN1 in CentralRegion

        List<Resource> relatedResource = ResourceUtils.getResourceList(e, ResourceType.RelatedPerson);
        assertThat(relatedResource).hasSize(0);  // No INSURANCE.IN1 in CentralRegion
    ...
    void testAdtA03AllSegmentsAndMultipleGroups() throws IOException {
        ...
        assertThat(e).hasSize(5);   // AL1, OBX, & IN1  (not supported in NZ CentralRegion)
        ...
        List<Resource> observationResource = ResourceUtils.getResourceList(e, ResourceType.Observation);
        assertThat(observationResource).hasSize(0); // from OBX  (not supported in NZ CentralRegion)

        List<Resource> allergyIntoleranceResource = ResourceUtils.getResourceList(e, ResourceType.AllergyIntolerance);
        assertThat(allergyIntoleranceResource).hasSize(0); // from AL1 (not supported in NZ CentralRegion)
        ...
        assertThat(insuranceResource).hasSize(0); //from INSURANCE.IN1   (not supported in NZ CentralRegion)

        List<Resource> organizationResource = ResourceUtils.getResourceList(e, ResourceType.Organization);
        assertThat(organizationResource).hasSize(0); //from INSURANCE.IN1   (not supported in NZ CentralRegion)
    ...
    void testAdtA03AllSegmentsNoGroupsPresent() throws IOException {
        ...
        assertThat(e).hasSize(3);  // AL1 & OBX   (not supported in NZ CentralRegion)
    ...
    void testAdtA03MinimumPlusInsuranceGroup() throws IOException {
        ...
        List<Resource> coverages = ResourceUtils.getResourceList(e, ResourceType.Coverage);
        assertThat(coverages).hasSize(0); //from INSURANCE.IN1   (not supported in NZ CentralRegion)

        List<Resource> organizationResource = ResourceUtils.getResourceList(e, ResourceType.Organization);
        assertThat(organizationResource).hasSize(0); //from INSURANCE.IN1   (not supported in NZ CentralRegion)

        List<Resource> relatedResource = ResourceUtils.getResourceList(e, ResourceType.RelatedPerson);
        assertThat(relatedResource).hasSize(0); //from INSURANCE.IN1    (not supported in NZ CentralRegion)

        // Confirm that there are no extra resources
        assertThat(e).hasSize(2);  // IN1    (not supported in NZ CentralRegion)
    ...
    void testAdtA05WithAllergiesMultipleProceduresAndSingleInsurance(String message) throws IOException {
        ...
        // Don't process OBX from ADT messages
        List<Resource> observationResource = ResourceUtils.getResourceList(e, ResourceType.Observation);
        assertThat(observationResource).hasSize(0); // from OBX x2

        // Don't process AL1
        List<Resource> allergyIntoleranceResource = ResourceUtils.getResourceList(e, ResourceType.AllergyIntolerance);
        assertThat(allergyIntoleranceResource).hasSize(0); // from AL1 x2
        ...
        // No Insurance
        List<Resource> coverages = ResourceUtils.getResourceList(e, ResourceType.Coverage);
        assertThat(coverages).hasSize(0); //from INSURANCE.IN1

        // No Insurance
        List<Resource> organizationResource = ResourceUtils.getResourceList(e, ResourceType.Organization);
        assertThat(organizationResource).hasSize(0); //from INSURANCE.IN1

        // No Insurance
        List<Resource> relatedResource = ResourceUtils.getResourceList(e, ResourceType.RelatedPerson);
        assertThat(relatedResource).hasSize(0); //from INSURANCE.IN1
    ...
    void testAdtA05MinimumWithNoProceduresAndMultipleInsurance(String message) throws IOException {
        ...
        // We don't do Insurance - so zero Coverage resources
        List<Resource> coverages = ResourceUtils.getResourceList(e, ResourceType.Coverage);
        assertThat(coverages).hasSize(0); //from INSURANCE.IN1 x2

        // We don't do Insurance - so zero Organization resources
        List<Resource> organizationResource = ResourceUtils.getResourceList(e, ResourceType.Organization);
        assertThat(organizationResource).hasSize(0); //from INSURANCE.IN1 x2

        // We don't do Insurance - so zero RelatedPerson resources
        List<Resource> relatedResource = ResourceUtils.getResourceList(e, ResourceType.RelatedPerson);
        assertThat(relatedResource).hasSize(0); //from INSURANCE.IN1 x2

        // Expecting 2 total resources
        // Patient, Encounter,  and none of Coverage, RelatedPerson, Organization
        assertThat(e).hasSize(2);
    }
