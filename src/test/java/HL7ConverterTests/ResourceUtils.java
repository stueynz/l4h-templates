...
import nz.govt.tewhatuora.central.l4h.CentralRegionHL7MessageEngine;

public class ResourceUtils {

    // We're doing testing - don't validate the FHIR Bundle - we need to see what's what while writing templates
    ...
    private static final HL7MessageEngine CUSTOM_ENGINE = new CentralRegionHL7MessageEngine(context, STANDARD_OPTIONS.getBundleType());

    public static List<BundleEntryComponent> createFHIRBundleFromHL7MessageReturnEntryList(HL7ToFHIRConverter ftv,
            String inputSegment) {
        return createFHIRBundleFromHL7MessageReturnEntryList(ftv, inputSegment, STANDARD_OPTIONS, CUSTOM_ENGINE);
    }

    public static List<BundleEntryComponent> createFHIRBundleFromHL7MessageReturnEntryList(HL7ToFHIRConverter ftv,
            String inputSegment, ConverterOptions options, HL7MessageEngine engine) {

        // Use the custom engine to get the required FHIR Bundle
        Bundle bundle = ftv.convertToBundle(inputSegment, options, engine);

        assertThat(bundle).isNotNull();
        LOGGER.debug("FHIR json result:\n" + engine.getFHIRContext().encodeResourceToString(bundle));
